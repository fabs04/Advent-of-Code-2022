...
function main()
    ...
    additional_required_space = required_space - free_space

    directories = collect(root_directory)

    result = root_directory

    for directory::Directory in directories
        if directory.size >= additional_required_space && directory.size < result.size
            result = directory
        end
    end
...
function collect(directory::Directory)::Array{Directory}
    result = [directory]

    for child in directory.children
        append!(result, collect(child))
    end
