...
function solve(valves)
    ...
    @variable(model, at[eachindex(valves), 1:30], Bin)
    @variable(model, move_to[eachindex(valves), 1:30], Bin)
    @variable(model, is_open[eachindex(valves), 1:30], Bin)
    @variable(model, open[eachindex(valves), 1:30], Bin)

    @objective(
        model,
        Max,
        sum(is_open[v,t] * valves[v].rate for v in eachindex(valves), t in 1:30)
    )
    ...
    for v in eachindex(valves)
        ...
        for t in 1:30
            ...
            if t > 1
                @constraint(model, is_open[v,t] <= sum(open[v,t2] for t2 in 1:t-1))

                @constraint(model, open[v,t] <= move_to[v,t-1])

                @constraint(model, at[v,t-1] <= at[v,t] + 1-sum(open[v1,t-1] for v1 in eachindex(valves)))
